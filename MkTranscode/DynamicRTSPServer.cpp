...
DynamicRTSPServer::createNew(UsageEnvironment& env, Port ourPort,
  ...
  DynamicRTSPServer* newDynamicRTSPServer = new DynamicRTSPServer(env, ourSocketIPv4, ourSocketIPv6, ourPort,
              authDatabase, reclamationTestSeconds);
  newDynamicRTSPServer->mpSubsession = NULL;
...
::lookupServerMediaSession(char const* streamName,
  ...
  if (strstr(streamName, "live") != NULL)
  {  
      fileExists = True;
      smsExists = True;
...
ServerMediaSession* DynamicRTSPServer::createNewSMS(UsageEnvironment& env,
					char const* fileName, FILE* /*fid*/) {
  // Use the file name extension to determine the type of "ServerMediaSession":
  char const* extension = strrchr(fileName, '.');
  printf("DynamicRTSPServer createNewSMS fileName = %s\n", fileName);
  if (strstr(fileName, "live") != NULL)
  {
    extension = "live";
  }
...
//#define INTERLEAVE_ADUS 1
    ...
    mpSubsession = MPEG2TransportLiveServerMediaSubsession::createNew(env, reuseSource);
    printf("DynamicRTSPServer Live Subsession = %p\n", mpSubsession);
    sms->addSubsession(mpSubsession);
  }

  return sms;
}

int DynamicRTSPServer::SubmitTsData(unsigned char* data, int size)
{
  //printf("DynamicRTSPServer::SubmitTsData data = %p, size = %d\n", data, size);
  if(mpSubsession) {
    mpSubsession->SubmitTsData(data, size);
  }
  return 0;
}